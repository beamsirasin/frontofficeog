...
3. เลือกไฟล์/โฟลเดอร์ทั้งหมด **ยกเว้น `node_modules` และ `.env`** (สำคัญ! ดูหมายเหตุด้านล่าง) แล้ว**ลากไปวางฝั่งขวา**
...
> 🛑 **ห้ามอัป `.env` จากเครื่องตัวเอง** เด็ดขาด (ถ้ามีไฟล์นี้อยู่ในโฟลเดอร์ตอนพัฒนาบนเครื่อง) — ไฟล์นั้นตั้งค่าไว้สำหรับพัฒนาบนเครื่อง (`COOKIE_SECURE=false`, `TRUST_PROXY=false`, รหัสผ่านทดสอบ) ถ้าอัปทับขึ้น VPS จริงจะทำให้ cookie ไม่ปลอดภัยและระบบกันโดนโจมตีรัวๆ (rate limit) เพี้ยนทั้งระบบเพราะแยกไม่ออกว่าใครเป็นใครหลัง nginx (ดู 3.3 ด้านล่าง — เราจะสร้าง `.env` ใหม่บน VPS แยกต่างหาก)
...
**3.3 สร้างไฟล์ `.env` บน VPS (สำคัญมาก — ห้ามข้ามขั้นนี้)**

ไม่มีไฟล์นี้ = **ล็อกอินไม่ได้เลยแม้แต่ครั้งเดียว** (ระบบจะไม่สร้างบัญชีเจ้าของร้านให้ถ้าไม่ตั้ง `ADMIN_USER`/`ADMIN_PASS`) และ cookie/การกันโดนโจมตีรัวๆ จะไม่ปลอดภัยเท่าที่ควรบน production

กลับไปที่ **cmd (SSH)**:
```bash
nano /root/frontofficeog/.env
```
วางนี้ (**แก้ `ADMIN_USER`/`ADMIN_PASS` เป็นของจริงที่จะใช้ล็อกอินครั้งแรกเสมอ** อย่าปล่อยเป็นค่าตัวอย่าง):
```
NODE_ENV=production
ADMIN_USER=เจ้าของร้าน
ADMIN_PASS=ตั้งรหัสผ่านที่คาดเดายากที่นี่
PORT=3000
...
> ⚠️ `COOKIE_SECURE=true` และ `TRUST_PROXY=true` **ต้องเป็น `true` บน production เท่านั้น** (มี HTTPS จริงผ่าน nginx แล้วในขั้นที่ 6) ถ้าเป็น `false` cookie จะไม่ถูกบังคับให้ส่งผ่าน HTTPS เท่านั้น และระบบกันโดนโจมตีรัวๆ (login, QR, ยกเลิกคิว) จะเห็นทุกคนเป็น IP เดียวกัน (ของ nginx เอง) ทำให้กันไม่ได้จริง — `NODE_ENV=production` เปิดสองค่านี้ให้อัตโนมัติอยู่แล้วในโค้ด แต่ใส่ทั้งสามบรรทัดไว้ชัดเจนกันลืม/กันพลาด
...
- [ ] เปิด `https://lumhimkhue.com/dashboard` → มีกุญแจ 🔒 → login ด้วย `ADMIN_USER`/`ADMIN_PASS` ที่ตั้งไว้ในขั้นที่ 3.3 ได้
